...
const ListingDetail: React.FC<ListingDetailProps> = ({ listingId }) => {
  ...
  const listingIdToUse = id || listingId;
  const isFavorite = listingIdToUse && listingIdToUse !== 'undefined' ? favorites.has(listingIdToUse) : false;
  ...
  useEffect(() => {
    const loadAd = async () => {
      const listingIdToUse = id || listingId;
      if (!listingIdToUse || listingIdToUse === 'undefined') {
        setError('Ungültige Anzeigen-ID');
        setLoading(false);
        return;
      }
      try {
        const adData = await adService.getAdById(listingIdToUse);
        if (!adData) {
        ...
        try {
          ...
          const similar = allAds
            .filter(a => a.id !== listingIdToUse && a.category === adData.category)
            .slice(0, 4);
          setSimilarListings(similar);
        } catch (error) {
